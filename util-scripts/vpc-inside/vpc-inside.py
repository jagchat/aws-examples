...
def formatter(prog): return HelpFormatter(prog, max_help_position=52)
...
parser.add_argument("-r", "--region", default="us-east-1",
                    help="AWS region that the VPC resides in")
...
def asg_in_vpc(asg):
    ...
    for subnet in subnets_list:
        try:
            sub_description = vpc_client.describe_subnets(SubnetIds=[subnet])[
                'Subnets']
            if sub_description[0]['VpcId'] == vpc_id:
                logger.info("{} resides in {}".format(
                    asg['AutoScalingGroupName'], vpc_id))
                return True
        except ClientError:
            pass
...
def describe_rdss():
    ...
    rdsss_list = [rds['DBInstanceIdentifier']
                  for rds in rdss if rds['DBSubnetGroup']['VpcId'] == vpc_id]
...
def describe_elbsV2():
    ...
    elbs_list = [elb['LoadBalancerArn']
                 for elb in elbs if elb['VpcId'] == vpc_id]
...
def describe_enis():
    enis = vpc_client.describe_network_interfaces(
